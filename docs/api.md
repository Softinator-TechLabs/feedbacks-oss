...
`category` is `general|visualDesign|productWorkflow|usabilityAccessibility`. Sort is `newest|activity|likes`; deviceClass is `mobile|tablet|desktop` (<600, <1000, >=1000 actual width). `state` is `open|in_progress|ready_for_review|resolved|declined`. Resolved/declined requires a nonempty note and permission. `kind` for evidence is `commit|pull_request|variant|incorporated_in`. Issue URL must be `https://github.com/OWNER/REPO/issues/NUMBER`; all Issue links are reported, never remotely verified. No create-Issue operation exists.
