...
Optional project QA uses `qa.configure {projectId,enabled,urls}` with one to three exact approved public HTTPS page URLs, `qa.get`, `qa.runNow` and `qa.runs`. The periodic worker checks public static HTML and bounded same-origin links; it does not create feedback or take screenshots. `qa.baselineSet {threadId,assetId}` chooses an existing private image from that thread, `qa.baselineGet` reads it and `qa.compare {threadId,assetId}` compares equal-sized existing images by pixel difference. Configuration and baseline changes require project maintainer access. Read/compare agent keys need explicit QA scopes; existing keys do not gain them. See [scheduled QA](scheduled-qa.md) for scan and comparison limits.
...
The [native app integration kit](mobile-sdk.md) uses existing paired-device credentials with `threads.create` and `assets.upload`. It sends a canonical HTTPS URL for the app screen, viewport size and a screenshot only after the host app's explicit preview/approval step. It does not add a new public operation or let guest links upload screenshots.
...
| `threads.list`           | `{projectId,limit?:30,offset?:0,search?:"",sort?:activity\|newest\|likes\|priority,showResolved?:false,url?,domain?,hostname?,deviceClass?}` | `{items:Thread[],total,nextOffset,websiteFilters:{domains,hostnames}}` |
...
| `threads.priority`       | `{threadId,revision,topPriority:boolean}`                                                                                                    | fresh `Thread`; project maintainer only                                |
...
`category` is `general|visualDesign|productWorkflow|usabilityAccessibility`. Sort is `newest|activity|likes|priority`; deviceClass is `mobile|tablet|desktop` (<600, <1000, >=1000 actual width). `state` is `open|in_progress|ready_for_review|resolved|declined`. Resolved/declined requires a nonempty note and permission. Human review decisions are independent of status; reopening starts a new round. `kind` for evidence is `commit|pull_request|variant|incorporated_in`. Issue URL must be `https://github.com/OWNER/REPO/issues/NUMBER`. Manual `threads.linkIssue` links remain `reported`. The optional GitHub App path verifies created Issues through GitHub readback and records `github_verified` without changing thread work status.

Current thread responses include `topPriority:boolean` for all authorized readers. In the policy-authorized `priority` sort, active manually marked threads precede other active threads, then each group follows its weighted `priorityScore`. The mark is a revision-checked maintainer write with a `threads.priority` audit event. Existing scoped agent keys do not acquire the new write; new keys need its explicit scope and project maintainer access.
...
Project maintainers can separately enable periodic two-way status sync with `github.statusSyncConfigure {projectId,revision,enabled:true}`. It considers only one `github_verified` Issue link in the connected repository per thread. The server checks at most 10 eligible threads per pass and revisits an eligible link no sooner than five minutes later. On first observation, matching states establish a baseline; a mismatch pauses for review. Thereafter a GitHub-only close resolves Feedbacks work, a GitHub-only reopen reopens a resolved thread, and a Feedbacks-only resolve/reopen closes or reopens the Issue. In-progress and ready-for-review both map to GitHub open; declined has no mapping. A change on both sides, a first mismatch, or an uncertain GitHub write pauses that thread. `github.statusSyncState {threadId}` exposes `pending|ready|conflict|uncertain|error|disabled` and the last agreed states. A maintainer reads both sides, then calls `github.statusSync {threadId,revision,issueUrl,source:"github"|"feedbacks"}` to choose the authoritative status. It requires an exact verified Issue in the currently connected repository; agent and extension keys cannot call it. Repeating an already-aligned choice avoids another GitHub PATCH or thread revision. Disabling sync or disconnecting stops new polls and writes; re-enabling establishes a fresh baseline. Existing Issue links and work history remain. Migration 17 adds the per-thread sync cursor and error state. A timed-out or unconfirmed PATCH remains `uncertain`; a repeated maintainer request does not send another PATCH until the observed GitHub state is reconciled. In-flight writes block competing choices. Automated writes that time out are never retried automatically. No incoming feedback creates an Issue through this path.

The thread's direct **Create Issue** action calls `github.issueCreateQuick {threadId,revision,idempotencyKey}`. It is for signed-in human maintainers only. The server transcribes the original feedback and adds durable, authenticated asset URLs. In a private repository it additionally signs image/video Wasabi URLs for seven days; public repositories receive no direct storage links. Anyone holding a signed URL can read the private object until expiry. The operation uses the same durable request reservation, readback and uncertain-result recovery as the edited path. The click is the maintainer's explicit approval to send the bounded text and links to GitHub; untrusted feedback never triggers creation on its own.

For a custom Issue, first read `threads.issueDraft {threadId}`, review and edit the title/body for accuracy and private content, then call `github.issueCreate {threadId,revision,reviewed:true,title,body,idempotencyKey}`. A signed-in human project maintainer can use the web interface. An agent may use the same operation through MCP only when a human owner issues a project-scoped key with the optional `github.issueCreate` scope. Existing keys and the one-click owner setup do not gain that scope. The GitHub App must already be installed and connected to the project repository. One native Issue request is allowed per thread. The server reserves the request before GitHub is called. A network failure or interrupted readback leaves it `pending`; repeating the request never sends another POST. Inspect the repository and ask a human maintainer to call `github.issueReconcile {threadId,revision,issueUrl}` for the actual Issue with the matching request marker, or, only after confirming no Issue exists and the ten-minute settlement window has passed, `github.issueAbandon {threadId,revision,confirmedAbsent:true}`. `github.issueState` reports when abandonment is available. `github.issueRefresh {threadId,revision,issueUrl}` reads current GitHub open/closed state for an already verified link, even if the project repository later changes. It does not change Feedbacks work or review status. Source: `src/server/github-operations.ts`, `tests/github-app.test.ts` and `tests/github-quick-issue.test.ts`.
...
Set `widget:true` on `guestProjectLinks.create` to receive `widgetSnippet` instead of a guest page path. A widget link requires exact approved project origins and has a separate token mode; it cannot open the standalone guest form. The snippet loads `/widget.js` on the approved website. Its token appears in page source, so treat it as a bounded public capability. `widget.inspect {linkId,token}` returns only project name and Turnstile site key. `widget.submit {linkId,token,name,body,url,viewport,turnstileToken}` creates one untrusted guest thread. Both widget operations require the link ID in the `linkId` query parameter and an `Origin` matching an exact approved project origin. Submission also requires the page URL to share that origin and a `widget_submit` Turnstile challenge verified for the host hostname. Revoke or exhaust the link to stop the widget; existing feedback is never returned. The host script does not capture screenshots; use the Chrome extension for private screenshot review.

### Project surveys

`surveys.create {projectId,title,description?,questions,expiresInDays?,maxResponses?}` requires a signed-in human project maintainer and configured Turnstile. It returns a one-time `token` and `/survey#token=...` path; neither `surveys.list {projectId}` nor `surveys.results {surveyId}` returns the token. Up to 10 active surveys per project may have 1–8 questions each. Question IDs are unique lowercase identifiers. Supported types are `nps` (integer 0–10), `rating` (integer 1–5), `single_choice` (2–8 distinct options) and `text` (at most 500 characters); each has a prompt and `required` flag. A survey expires after 1–90 days and accepts at most 1–1000 responses. Questions are immutable after creation. `surveys.revoke {surveyId}` stops new visitors; existing results remain.

Public `survey.inspect {token}` returns only the project name, frozen definition, expiry and Turnstile site key. `survey.submit {token,answers,responseKey,turnstileToken}` accepts one anonymous answer set after a `survey_submit` challenge; it enforces the definition, remaining capacity and a per-IP ingress limit. `responseKey` is a stable client-generated retry key. The service stores no name, email or IP with responses. Maintainers read aggregate counts, NPS and up to 100 recent written answers per question through `surveys.results`; other project roles have no results access. NPS is `% promoters (9–10) − % detractors (0–6)`, rounded to an integer, or null without answers. Surveys do not create threads or appear in thread exports.
...
Project documents use `documents.upload {projectId,name,fileBase64,idempotencyKey}`. Only a signed-in human project maintainer can upload; the server accepts canonical base64 for a PDF, PNG, JPEG or WebP up to 8 MiB, checks PDF parsing and its 1–25 page count, and normalizes images to WebP. The response has `{id,projectId,name,kind,contentType,pageCount,bytes,createdAt,url}` and page dimensions when available. `documents.list {projectId}` and `documents.get {documentId}` require current project access. `documents.threads {documentId,page,cursor?}` returns up to 100 newest-first markers for that document page and `nextCursor` for older markers; a cursor is valid only for the same document page. The private source path `/api/documents/:id/file` requires a current cookie or bearer token with `documents.get` scope and serves downloads with `Cache-Control:no-store` and `Content-Disposition:attachment`. A document-scoped `threads.create` requires exactly one of `context` and `document`; coordinates are normalized from 0 to 1 and the page must belong to that document. All document threads remain ordinary discussions. Existing scoped token presets are unchanged; a new key needs explicit document read scopes. Guest links cannot access documents.
...
- `sort:"priority"` gives policy-authorized users and agents the same queue shown by the inbox's **Top priority** button. Active feedback comes first, then current author importance for that feedback category plus current policy-weighted likes on its captured view. Ties use latest activity, then ID. This is a review aid, not an automatic decision or status change. The returned items include `priorityScore` only for this sort. Calls without owner or explicit `context.policy` authority are rejected; the queue never exposes reviewer policy through ordinary list access.
