...
Set `widget:true` on `guestProjectLinks.create` to receive `widgetSnippet` instead of a guest page path. A widget link requires exact approved project origins and has a separate token mode; it cannot open the standalone guest form. The snippet loads `/widget.js` on the approved website. Its token appears in page source, so treat it as a bounded public capability. `widget.inspect {linkId,token}` returns only project name and Turnstile site key. `widget.submit {linkId,token,name,body,url,viewport,turnstileToken,screenshot?}` creates one untrusted guest thread and optionally a private WebP asset. The screenshot is a PNG/JPEG/WebP data URI at most 3 MiB encoded. Both widget operations require the link ID in the `linkId` query parameter and an `Origin` matching an exact approved project origin. Submission also requires the page URL to share that origin and a `widget_submit` Turnstile challenge verified for the host hostname. Revoke or exhaust the link to stop the widget; existing feedback is never returned.
