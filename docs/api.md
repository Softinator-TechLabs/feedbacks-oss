...
Set `GITHUB_APP_ID`, `GITHUB_APP_SLUG` and `GITHUB_APP_PRIVATE_KEY_BASE64` together on the server. The key is a base64-encoded RSA private key and belongs only in deployment secrets. Grant the App repository metadata read and Issues read/write, then install it on the intended repository. In project settings, save an exact `https://github.com/OWNER/REPO` URL and select **Connect repository**. Existing MCP agent handoff needs none of these settings.
