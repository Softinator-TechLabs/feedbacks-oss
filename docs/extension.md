...
For an element-specific review, right-click the element. Feedbacks outlines it and opens a comment field beside it. Write a note and choose **Save point**, then right-click another element to add more numbered notes. Use **Review screenshots** when finished; the editor shows every note and its order, with an optional overall comment and the drawing/redaction tools. A review across different scroll positions captures numbered full-page screenshots so every point can be placed on its section. Each point's text and element context remain linked in the resulting thread, API and agent tools. If the element changes before capture, check the saved point and the screenshot before sending.

When a reviewer opens the same page again, saved pins appear on confidently matched elements. Hover or focus a pin to read its specific note. In the Feedbacks thread, **Review on the page** shows numbered points over the approved screenshot, their complete notes, and a screenshot selector. Tap a point or choose **Show on screenshot** on mobile. Pencil, arrow, rectangle and text marks remain visible in the approved image; the thread also summarizes their types.
...
Each screenshot asset records its filename, page range and normalized point/drawing locations. The optional combined asset also records each retained source section's page range and position within the merged image, so removed sections are not mistaken for continuous page content. Agents should use the separate numbered images for readable visual detail and the metadata to connect a mark to its comment.
