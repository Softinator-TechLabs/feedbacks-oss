...
In authenticated Help, **Copy agent setup** explicitly creates and copies a new 90-day owner-administration key. Its notice explains full current/future project and account access, including private notes for the primary owner's delegation. Zero projects is valid; the agent can create the first project on a later explicit request. Non-owners see an owner-help path. Clipboard denial retains the same issued secret in tab memory for retry/manual copy. Opening Help never issues a token. Account **Advanced** remains the ordinary project-scoped setup described above.
